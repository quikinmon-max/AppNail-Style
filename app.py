...
from bson.objectid import ObjectId # NUEVO: Para poder actualizar datos de usuarios
import pandas as pd
...
# --- UTILIDADES ---
# Le agregué un tamaño un poco más grande para que los fondos se vean con buena calidad
...
# ==========================================
    ...
    # 1. Inyectar Personalización (White-Label) Dinámica
    fondo_val = st.session_state.get('fondo', '#0d0d0d')
    
    # Si no empieza con "#", significa que es una imagen en Base64
    if fondo_val and not fondo_val.startswith("#"):
        css_fondo = f"background-image: url('data:image/png;base64,{fondo_val}'); background-size: cover; background-attachment: fixed; background-position: center;"
    else:
    ...
    st.markdown(f"""
        ...
        div[data-testid="stForm"], div[data-testid="stExpander"] {{ background-color: rgba(0, 0, 0, 0.8); border-radius: 10px; padding: 15px; border: 1px solid #4b0082; }}
        </style>
        """, unsafe_allow_html=True)
    ...
    opcion = st.sidebar.radio("Navegación", 
                             ["Buscador por Nombre", 
                              ...
                              "🎨 Personalizar Estética"]) # NUEVA OPCIÓN
    ...
    # --- MÓDULO 5: PERSONALIZACIÓN (EL NUEVO TOQUE PREMIUM) ---
        ...
        with st.form("form_personalizacion"):
            ...
            if tipo_fondo == "Imagen / Foto":
                st.write("Sube una foto (Ej: Textura de mármol, escarcha, tu salón).")
                nuevo_fondo_file = st.file_uploader("Subir Imagen de Fondo", type=["jpg", "png", "jpeg"])
            else:
                # Recuperar el color actual si es que tienen un color guardado
            ...
            if st.form_submit_button("Guardar Diseño"):
                # Preparamos el diccionario con lo que vamos a actualizar en MongoDB
                datos_a_actualizar = {}
                ...
                if datos_a_actualizar:
                    # Actualizamos la colección "usuarios" usando el ID de la sesión actual
                    usuarios_col.update_one(
                        {"_id": ObjectId(st.session_state['usuario_id'])},
                        {"$set": datos_a_actualizar}
                    )
                    st.success("¡Diseño actualizado exitosamente!")
                    st.rerun() # Recargamos para que el CSS se aplique al instante
                else:
